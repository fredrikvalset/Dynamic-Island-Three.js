...
import { PlaneBufferGeometry } from '../library/three.module.js';
...
export default class TerrainBufferGeometry extends PlaneBufferGeometry {
  constructor({
    ...
    if (heightmapImage) {
      // get heightmap data
      this.heightmap = Utils.getHeightmapData(
        heightmapImage,
      ...
      for (var i = 0; i <= this.attributes.position.count; i++) {
        this.attributes.position.setY(
          i,
          height * ((noise[i] - min) / (max - min))
        );
      }
    } else {
      throw Error(
        'Unable to create TerrainBufferGeometry, must receive heightmapImage or noise function.'
      );
    }
   ...
   */
  getY(x, z) {
    return this.attributes.position.getY(
      Math.trunc(z) * (this.numberOfSubdivisions + 1) + Math.trunc(x)
    );
  }
